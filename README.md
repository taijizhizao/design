...
理想情况下，我们希望后台采用面向服务的架构方式（SOA），后台仅提供单纯的业务数据，不要去考虑用户认证、日志处理等与业务无关的内容。
...
考虑到nodejs对mysql数据库有良好的支持，我们需维护一个mysql数据库供nodejs使用，在其中维护用户、菜单、消息、日志等数据。如果后台使用的数据库也是mysql，则可以把两个数据库合并到一起。
